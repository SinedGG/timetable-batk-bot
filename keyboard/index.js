      ...
      db.query(
        ...
        function (err) {
          if (err) {
            ...
            console.log("Group", `Встановлено розклад-таблицю для користувача ${ctx.chat.id}`, err);
    ...
    if(ctx.callbackQuery.data == "NN"){
      db.query(
        ...
        function (err, setting) {
          if (err) {
            console.log("DB Error", `помилка отримання notification_n, notification_z для крористувача ${ctx.chat.id}`, err);
          } else {
            if (setting[0].notification_n) {
              db.query(
                ...
                function (err) {
                  if (err) {
                    console.log("DB Error", `помилка оновлення notification_n=false для крористувача ${ctx.chat.id}`, err);
                  } else {
                    set1 = "❌";
                    ...
                    console.log("Notification", `Cповіщення нового розкладу вимкнуто для користувача ${ctx.chat.id}`, err);
                  }
                }
              );
            } else {
              db.query(
                ...
                function (err) {
                  if (err) {
                    console.log("DB Error", `Помилка оновлення notification_n=true для крористувача ${ctx.chat.id}`, err);
                  } else {
                    set1 = "✔️";
                    if (setting[0].notification_z) {
                      set2 = "✔️";
                    ...
                    EditInline(ctx, set1, set2);
                    console.log("Notification", `Cповіщення нового розкладу увімкнено для користувача ${ctx.chat.id}`, err);
                  }
                }
              );
            }
          ...
          if (err) {
            ...
            if (setting[0].notification_z) {
              db.query(
                ...
                function (err) {
                  if (err) {
                    console.log("DB Error", `помилка оновлення notification_z=false для крористувача ${ctx.chat.id}`, err);
                  } else {
                    set2 = "❌";
                    if (setting[0].notification_n) {
                      set1 = "✔️";
                    ...
                    EditInline(ctx, set1, set2);
                    console.log("Notification", `Cповіщення змін в розкладі вимкнуто для користувача ${ctx.chat.id}`, err);
                  }
                }
              );
            } else {
              db.query(
                ...
                function (err) {
                  if (err) {
                    ...
                    EditInline(ctx, set1, set2);
                    console.log("Notification", `Cповіщення змін в розкладі увімкнено для користувача ${ctx.chat.id}`, err);
                  }
                }
              );
            }
